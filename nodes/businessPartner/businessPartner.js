...
module.exports = function (RED) {
    function businessPartner(config) {
        RED.nodes.createNode(this, config);
        let node = this;
        node.on('input', async function (msg) {
            try {
                node.status({ fill: 'grey', shape: 'dot', text: 'Requesting...' });
                let result = await reqEngine.businessPartner(node, msg._LY1_Service, config.typeReq, msg.payload);
                msg.payload = result;
                if (!result.success) {
    ...
    RED.nodes.registerType("businessPartner", businessPartner);
}
